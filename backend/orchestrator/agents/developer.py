    ...
    def _build_prompt(
        ...
        *,
        ...
        return (
            ...
            "1. Output ONLY a valid git unified diff - NO prose, NO explanations, NO markdown, NO code fences\n"
            "2. MANDATORY prefixes: a/ and b/ for all file paths (e.g., a/resources/js/file.js b/resources/js/file.js)\n"
            "3. Each file MUST have complete headers in this EXACT order:\n"
            "   - diff --git a/<path> b/<path>\n"
            "   - (optional) new file mode 100644 or deleted file mode 100644\n"
            "   - (optional) index <hash>..<hash>\n"
            "   - --- a/<path> or --- /dev/null (for new files)\n"
            "   - +++ b/<path> or +++ /dev/null (for deleted files)\n"
            "4. Hunk headers @@ -old_start,old_count +new_start,new_count @@ MUST match actual line counts\n"
            "5. Context lines start with space, additions with +, deletions with -\n"
            "6. NO smart quotes (" " ' '), NO CRLF line endings (use LF only)\n"
            "7. Maximum 3 files per patch - split larger changes into multiple steps\n\n"
            "VALID EXAMPLE (new file):\n"
            "diff --git a/resources/js/slider.js b/resources/js/slider.js\n"
            "new file mode 100644\n"
            "index 0000000..abc1234\n"
            "--- /dev/null\n"
            "+++ b/resources/js/slider.js\n"
            "@@ -0,0 +1,3 @@\n"
            "+export default function initSlider() {\n"
            "+    console.log('Slider initialized');\n"
            "+}\n"
            "\n"
            "VALID EXAMPLE (modify existing file):\n"
            ...
            " }\n"
            "\n"
            "Wrap ONLY the raw diff between these markers:\n"
            "BEGIN_PATCH\n"
            "<pure git unified diff following ALL rules above>\n"
            "END_PATCH\n"
        )
